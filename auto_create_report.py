...
def build_comment_ui(author, dt_local, parsed_html, color_hex, is_history=False):
    ...
    html += f"<strong>🗣️ {author}</strong> <span style='color: #666; font-size: 0.85em;'>({
            dt_local.strftime('%Y-%m-%d %H:%M')})</span>" if c_summary:
        tags_html = ""
        if c_tags:
...
def run_daily_snapshot():
    ...
    print(f"🗓️  Generating HTML Snapshot | Today: {t
    ...
    def generate_section(title_html, epics_data, target_date_str, is_yesterday=False):
        ...
        for epic_key, epic_data in epics_data.items():
            ...
            html += f"🔷 {epic_link} {e_sum_display} <span style='font-weight: normal; font-size: 0.8em; color: {
                # ffffff;'>"
                border_color};'>({len(epic_data['tasks'])} tasks)</span></summary>"            html += f"<div style='padding: 15px; background-color:
            ...
            for task in epic_data["tasks"]:
                ...
                html += f"<summary style='cursor: pointer; padding: 10px; background-color: #f4f5f7; font-weight: bold; font-size: 1.0em; outline: none;'>🛠️ <a href='https://{A
                                                                                                                                                                               ATLASSIAN_DOMAIN}/browse/{t_key}' target='_blank'>[{t_key}]</a> {t_sum_display}</summary>"                html += f"<div style='padding: 15px; background-color: #ffffff;'>"
    ...
    # 1. TODAY
    generate_section(f"<h2 style='background: #0052cc; color: white; padding: 10px; border-radius: 4px;'>📅 TODAY ({t
    ...
    if pending_epics:
        for epic in pending_epics:
            ...
            html += f"<details class='desc-collapse epic-desc'><summary>📄 View Epic Description</summary><div style='padding: 10px 15px; background-color: #ffffff; font-size: 0.95em;'>{
                    parsed_epic_desc}</div></details></div></details>"    else:
    ...
    with open(filename, "w", encoding="utf-8") as f:
        f.write(html)
    print("-" * 60 + f"\n🏁 HTML Snapshot complete! Open '{filename}'.")
