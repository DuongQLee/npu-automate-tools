...
def convert_adf_to_html(node, attachment_map):
    ...
    elif node_type in ["mediaSingle", "mediaGroup"]: return f'<div>{inner_html}</div>'
...
def run_daily_snapshot():
    ...
    def build_epic_map(epics, tasks):
        ...
        emap["OTHER"] = {"summary": "Standalone Tasks", "status": "", "description": None, "attachments": [], "tasks": []}
        for t in tasks:
    ...
    def generate_html_section(title, emap, target_date):
        section_html = f"<h2 style='background: #0052cc; color: white; padding: 10px; border-radius: 4px;'>{title}</h2>"
        for epic_key, epic_data in emap.items():
            ...
            section_html += f"<details open style='margin-bottom: 20px; border: 2px solid #0052cc; border-radius: 6px;'><summary style='padding: 15px; background-color: #deebff; font-weight: bold;'>🔷 {epic_link} {e_sum_display} ({len(epic_data['tasks'])} tasks)</summary><div style='padding: 15px;'>"
            
            if epic_key != "OTHER":
                parsed_epic_desc = convert_adf_to_html(epic_data["description"], {}) if epic_data["description"] else "<em>No description.</em>"
                section_html += f"<details><summary>📄 View Epic Description</summary><div>{parsed_epic_desc}</div></details>"
            ...
            for task in epic_data["tasks"]:
                ...
                section_html += f"<details style='margin-bottom: 10px; border: 1px solid #dfe1e6; border-radius: 4px;'><summary style='padding: 10px; background-color: #f4f5f7;'>🛠️ <a href='https://{ATLASSIAN_DOMAIN}/browse/{t_key}'>[{t_key}]</a> {t_sum_display}</summary><div style='padding: 10px;'>"
                
                comments = fetch_comments(t_key)
                todays_comments = "".join([build_comment_ui(c["author"]["displayName"], datetime.strptime(c["created"], "%Y-%m-%dT%H:%M:%S.%f%z").astimezone(), convert_adf_to_html(c["body"], {}), "#36b37e") for c in comments if datetime.strptime(c["created"], "%Y-%m-%dT%H:%M:%S.%f%z").astimezone().strftime('%Y-%m-%d') == target_date])
    ...
    html += generate_html_section(f"📅 TODAY ({today_str})", epics_map, today_str)
    html += generate_html_section(f"⏪ YESTERDAY ({yesterday_str})", yest_epics_map, yesterday_str)
    html += "</body></html>"
    ...
    confluence_html = re.sub(r'<head>.*?</head>|<!DOCTYPE html>|<html>|</html>|<body>|</body>|<script.*?>.*?</script>', '', html, flags=re.IGNORECASE | re.DOTALL)
